...
def run_pb_image():
    ...
    # imgs = glob(r"G:\data\fu_fprs21c\02_dataset_ori\image\*\*.jpg")
    imgs = glob(r"d:/data/lips/videos/20220321/*/*.jpg")
    print(len(imgs),imgs[:10])
    # "G:\data\sjt_fprs21x\02_dataset_ori\data\category_1"
    # "G:\data\sjt_fprs21x\02_dataset_ori\data\category_2"
    imgs.sort()
    # imgs = glob(r"./imgs/*.jpg")
    key = 1
    ...
    for path in imgs:
        ...
        for i in range(det_boxes.shape[0]):
            ...
            np.save(path[:-4]+".npy", Landmark_arr)
            ##################draw face points######################
            # contour = arange(0, 15)
            # nose = np.append(arange(35, 46), 64)
            # array = np.concatenate((nose, contour))
...
def run_pb_video():
    ...
    videos = glob(r"/mnt/fu04/xueluoyang/git/video_img_face_landmark_detect/video_20220110/*.mp4")

    print(len(videos),videos[:10])
    ...
    for n_v,video_path in enumerate(videos):
        ...
        while(success):
            if tracking is False:
                Landmark = []
                det_boxes, det_landmarks = mtcnn_detector.detect(np.array(frame))
                print("det_landmarks",det_landmarks)
                if len(det_boxes) > 0:
                    det = det_boxes[0, :4]
